...
class SparkyBench(Node):
    def __init__(self, args, outdir):
        super().__init__("sparky_nav2_benchmark_v11_14")
        self.args = args
        self.outdir = outdir
...
def verify_runtime_contract():
    """Fail fast unless the running controller matches the ROS 2 Jazzy flat namespace."""
    required = {
        "FollowPath.plugin": ("string", "nav2_rotation_shim_controller::RotationShimController"),
        "FollowPath.primary_controller": ("string", "dwb_core::DWBLocalPlanner"),
        "FollowPath.rotate_to_heading_angular_vel": ("number", 0.80),
        "FollowPath.max_angular_accel": ("number", 5.0),
        "FollowPath.max_vel_x": ("number", 0.35),
        "FollowPath.min_speed_xy": ("number", 0.20),
        "FollowPath.max_vel_theta": ("number", 1.00),
        "FollowPath.min_speed_theta": ("number", 0.35),
        "FollowPath.acc_lim_theta": ("number", 5.0),
        "FollowPath.decel_lim_theta": ("number", -5.0),
        "FollowPath.closed_loop": ("bool", False),
        "FollowPath.rotate_to_heading_once": ("bool", True),
        "general_goal_checker.xy_goal_tolerance": ("number", 0.10),
        "general_goal_checker.yaw_goal_tolerance": ("number", 0.20),
    }

    problems = []
    observed = {}

    for param, (kind, expected) in required.items():
        try:
            p = subprocess.run(
            ...
            if p.returncode != 0 or "Parameter not set" in out or "Parameter not declared" in out:
                problems.append(f"{param}: unavailable ({out})")
                continue

            if kind == "string":
                if str(expected) not in out:
                    problems.append(f"{param}: expected {expected!r}, got {out!r}")
            elif kind == "bool":
                low = out.lower()
                got = None
                if "true" in low:
                ...
                if got is None or got != expected:
                    problems.append(f"{param}: expected {expected}, got {out!r}")
            else:
                m = re.search(
                ...
                if not m:
                    problems.append(f"{param}: could not parse numeric value from {out!r}")
                    continue
                got = float(m.group(1))
