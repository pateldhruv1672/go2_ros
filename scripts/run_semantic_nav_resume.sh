...
BASE_READY=0
if ros2 node list --disable-daemon 2>/dev/null | grep -q "^/go2_driver_node$"; then
  BASE_READY=1
fi
...
if [ "$BASE_READY" -eq 0 ]; then
  ...
  for _ in $(seq 1 45); do
    if ros2 node list --disable-daemon 2>/dev/null | grep -q "^/go2_driver_node$" && \
       ros2 topic list --disable-daemon 2>/dev/null | grep -q "^/odom$" && \
       ros2 topic list --disable-daemon 2>/dev/null | grep -q "^/scan$"; then
