...
class FastSAM2TrackerOverlayNode(Node):
    ...
    def publish_detections(self, detections: List[Dict[str, Any]], image_width: int, image_height: int, source_stamp_sec: float):
        msg = String()
        msg.data = json.dumps(
            {
                "stamp_sec": float(source_stamp_sec) if source_stamp_sec > 0.0 else time.time(),
