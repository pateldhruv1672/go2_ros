from glob import glob as _raw_glob
import os
...
# SPARKY_SAFE_DATA_GLOB_V1
def glob(pattern):
    """Return only real files; ignore stale/dangling symlinks in package resources."""
    return [p for p in _raw_glob(pattern) if os.path.isfile(p)]
