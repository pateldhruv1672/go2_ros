...
class PoseAwareObjectMapper(Node):
    ...
    def publish_map(self) -> None:
        objects = [asdict(rec) for rec in sorted(self.objects.values(), key=lambda r: r.object_id)]
        ...
        for rec in self.objects.values():
            ...
            status = "confirmed" if rec.confirmed else "tentative"
            text.text = f"{rec.label} #{rec.object_id} {status} views={rec.confirmations}"
